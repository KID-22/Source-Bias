...
class Rerank:
    ...
    def rerank(self, 
        ...
        rerank_scores = [score for score in self.cross_encoder.predict(sentence_pairs, batch_size=self.batch_size)]
        if type(rerank_scores[0]) == type(np.array([0,0])):
            rerank_scores = [float(score[0]) for score in rerank_scores]
        else:
