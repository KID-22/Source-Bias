...
class EvaluateRetrieval:
    ...
    def evaluate(qrels: Dict[str, Dict[str, int]], 
        ...
        # cal the acc on human vs LLM
        rates = {}
        for k in k_values:
            rates[f"RATE@{k}"] = 0.0
        for query_id, search_dict in results.items():
            sorted_items = sorted(search_dict.items(), key=lambda x: x[1], reverse=True)
            sorted_search = {item[0]: item[1] for item in sorted_items}
            flags = []
            # if len(sorted_search) == 0:
            #     flags.append(0.5)
            for i in range(min(len(sorted_search), k_values[-1])):
                flags.append(1)
                for source in ["gpt", "llama", "human"]:
                    if source in list(sorted_search.keys())[i]:
                        flags[-1] = 0
                        break
            for k in k_values:
                rates[f"RATE@{k}"] += sum(flags[:k])/min(len(flags), k)
        for k in k_values:
            rates[f"RATE@{k}"] = round(rates[f"RATE@{k}"]/len(results), 5)
    
        for eval in [ndcg, _map, recall, precision, rates]:
            logger.info("\n")
            for k in eval.keys():
