...
class SentenceBERT:
    ...
    def encode_corpus(self, corpus: Union[List[Dict[str, str]], Dict[str, List]], batch_size: int = 8, **kwargs) -> Union[List[Tensor], np.ndarray, Tensor]:
        ...
        #     sentences = [(corpus["title"][i] + self.sep + corpus["text"][i]).strip()[:max_len] if "title" in corpus else corpus["text"][i].strip()[:max_len] for i in range(len(corpus['text']))]
        # else:
        #     sentences = [(doc["title"] + self.sep + doc["text"]).strip()[:max_len] if "title" in doc else doc["text"].strip()[:max_len] for doc in corpus] 
        if type(corpus) is dict:
